export default async function handler(request: Request, context: any) {
  ...
  // Check for sitemap index request
  const url = new URL(request.url);
  const isIndex = url.searchParams.get('index') === 'true';
  const page = parseInt(url.searchParams.get('page') || '0');
  const URLS_PER_SITEMAP = 1000; // Google recommends max 50,000 but smaller is better for crawling
  ...
  try {
    ...
    console.log(`Total URLs in dynamic sitemap: ${urls.length}`);
